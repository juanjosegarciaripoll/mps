...
template <typename Tensor>
inline MPS<Tensor> product_state(index_t length, const Tensor &local_state) {
...
extern template double expected(const RMPS &a, const RTensor &Op1, index_t k,
                                int direction);
...
/**Compute a single-site expected value.*/
extern template cdouble expected(const CMPS &a, const CTensor &Op1, index_t k,
                                 int direction);
...
extern template double expected(const RMPS &a, const RTensor &op1, index_t k1,
                                const RTensor &op2, index_t k2, int direction);

/**Compute a two-site correlation.*/
extern template cdouble expected(const RMPS &a, const CTensor &op1, index_t k1,
                                 const CTensor &op2, index_t k2, int direction);

/**Compute a two-site correlation.*/
extern template cdouble expected(const CMPS &a, const CTensor &op1, index_t k1,
                                 const CTensor &op2, index_t k2, int direction);
...
void set_canonical(RMPS &psi, index_t site, const RTensor &A, int sense,
                   bool truncate = true);

/**Store a tensor in a matrix product state in the canonical form.*/
void set_canonical(CMPS &psi, index_t site, const CTensor &A, int sense,
                   bool truncate = true);
