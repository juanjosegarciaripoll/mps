...
inline tensor_scalar_t<tensor_common_t<T1, T2>> scprod(
  ...
  if (direction == DIR_RIGHT) {
    for (index_t k = 0; k < a.ssize(); k++) {
      env = env.propagate(a[k], b[k]);
    }
  } else {
    for (index_t k = a.ssize(); k--;) {
      env = env.propagate(a[k], b[k]);
    }
  }
  return env.close();
...
inline tensor_scalar_t<tensor_common_t<T1, T2>> expected(
    const MPS<T1> &a, const T2 &op, index_t k1, int direction = DIR_RIGHT) {
  ...
  for (index_t k = 0, target = a.normal_index(k1); k < a.ssize(); k++) {
    auto Pk = a[k];
    if (k == target) {
...
template <class T1, class T2>
inline tensor_scalar_t<tensor_common_t<T1, T2>> expected(
    const MPS<T1> &a, const T2 &op1, index_t k1, const T2 &op2, index_t k2,
