...
CMPS apply_H_Trotter3(const Hamiltonian &H, double dt, const CMPS &psi) {
...
void test_Trotter3_no_truncation(const Hamiltonian &H, double dt,
                                 const CMPS &psi) {
  CMPS aux = psi;
...
TEST(Trotter3Solver, LocalOperatorSzFull) {
  test_over_integers(2, 5, evolve_local_operator_sz,
                     test_Trotter3_no_truncation);
}

TEST(Trotter3Solver, LocalOperatorSzTruncated) {
  test_over_integers(2, 5, evolve_local_operator_sz,
                     test_Trotter3_truncated<2>);
}
