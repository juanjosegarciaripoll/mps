...
namespace tensor_test {
  ...
  TEST(AnyLatticeTest, EmptyLattice) {
	...
	for (int j = 0; j < size; j++) {
	  EXPECT_TRUE(all_equal(zero, L.hopping_operator(j,i,Lattice::HARD_CORE_BOSONS)));
	  EXPECT_TRUE(all_equal(zero, L.hopping_operator(j,i,Lattice::FERMIONS)));
  ...
  TEST(AnyLatticeTest, FullLattice) {
	...
	for (int j = 0; j < size; j++) {
	  if (i == j) {
	    EXPECT_TRUE(all_equal(one, L.hopping_operator(j,i,Lattice::HARD_CORE_BOSONS)));
	    EXPECT_TRUE(all_equal(one, L.hopping_operator(j,i,Lattice::FERMIONS)));
	  } else {
	    EXPECT_TRUE(all_equal(zero, L.hopping_operator(j,i,Lattice::HARD_CORE_BOSONS)));
	    EXPECT_TRUE(all_equal(zero, L.hopping_operator(j,i,Lattice::FERMIONS)));
	  }
  ...
  TEST(AnyLatticeTest, OneParticleLattice) {
	...
	for (int j = 0; j < size; j++) {
	  RTensor hop = zero;
	  hop.at(j,i) = 1.0;
	  EXPECT_TRUE(all_equal(hop, L.hopping_operator(j,i,Lattice::HARD_CORE_BOSONS)));
	  EXPECT_TRUE(all_equal(hop, L.hopping_operator(j,i,Lattice::FERMIONS)));
	  RTensor inter = zero;
	  inter.at(j,i) = (i == j);
  ...
  TEST(HCBLatticeTest, OneInTwo) {
    ...
    // Configurations: 01, 10
    {
      ...
      hop0to1.at(1,0) = 1.0;
      EXPECT_TRUE(all_equal(hop0to1, L.hopping_operator(1,0,Lattice::HARD_CORE_BOSONS)));
      EXPECT_TRUE(all_equal(hop0to1, L.hopping_operator(1,0,Lattice::FERMIONS)));

      RTensor hop1to0 = adjoint(hop0to1);
      EXPECT_TRUE(all_equal(hop1to0, L.hopping_operator(0,1,Lattice::HARD_CORE_BOSONS)));
      EXPECT_TRUE(all_equal(hop1to0, L.hopping_operator(0,1,Lattice::FERMIONS)));
  ...
  TEST(HCBLatticeTest, OneInThree) {
    ...
    }
    {
      ...
      hop0to2.at(2,0) = 1.0;
      EXPECT_TRUE(all_equal(hop0to2, L.hopping_operator(2,0,Lattice::HARD_CORE_BOSONS)));
      RTensor hop2to0 = adjoint(hop0to2);
      EXPECT_TRUE(all_equal(hop2to0, L.hopping_operator(0,2,Lattice::HARD_CORE_BOSONS)));
    }
    {
      RTensor hop1to2 = RTensor::zeros(3,3);
      hop1to2.at(2,1) = 1.0;
      EXPECT_TRUE(all_equal(hop1to2, L.hopping_operator(2,1,Lattice::HARD_CORE_BOSONS)));
      RTensor hop2to1 = adjoint(hop1to2);
      EXPECT_TRUE(all_equal(hop2to1, L.hopping_operator(1,2,Lattice::HARD_CORE_BOSONS)));
    }
    {
  ...
  TEST(FermionLatticeTest, TwoInThree) {
    ...
    }
    {
      RTensor hop0to2 = RTensor::zeros(3,3);
      hop0to2.at(2,0) = -1.0;
      EXPECT_TRUE(all_equal(hop0to2, L.hopping_operator(2,0,Lattice::FERMIONS)));
      RTensor hop2to0 = adjoint(hop0to2);
      EXPECT_TRUE(all_equal(hop2to0, L.hopping_operator(0,2,Lattice::FERMIONS)));
    }
    {
      RTensor hop1to2 = RTensor::zeros(3,3);
      hop1to2.at(1,0) = 1.0;
      EXPECT_TRUE(all_equal(hop1to2, L.hopping_operator(2,1,Lattice::FERMIONS)));
      RTensor hop2to1 = adjoint(hop1to2);
      EXPECT_TRUE(all_equal(hop2to1, L.hopping_operator(1,2,Lattice::FERMIONS)));
    }
  }
  ...
  TEST(FermionLatticeTest, TwoInFour) {
    ...
    }
    {
      ...
      hop1to3.at(3,0) = 1.0;
      hop1to3.at(5,2) = -1.0;
      EXPECT_TRUE(all_equal(hop1to3, L.hopping_operator(3,1,Lattice::FERMIONS)));
      RTensor hop3to1 = adjoint(hop1to3);
      EXPECT_TRUE(all_equal(hop3to1, L.hopping_operator(1,3,Lattice::FERMIONS)));
    }
  }
