...
CTensor arnoldi_expm(const Ham &H, const Tensor &psi, cdouble idt,
  ...
  if (max_states <= 0) {
    std::cerr
        << "In arnoldi_expm(), max_states must be a non-negative number.\n";
    abort();
  }
  ...
  for (int i = 0; i < coef.size(); i++) output = output + states[i] * coef[i];
  return output;
}
...
TEST(ArnoldiSolver, LocalOperatorSzTruncated) {
  test_over_integers(2, 7, evolve_local_operator_sz,
                     test_Arnoldi_truncated<4, 6>);
}

TEST(ArnoldiSolver, LocalOperatorSxTruncated) {
  test_over_integers(2, 7, evolve_local_operator_sx,
                     test_Arnoldi_truncated<4, 5>);
}
