...
template <class MPO>
void test_default_mpo(int size) {
  ...
  EXPECT_CEQ(mpo_to_matrix(mpo), Tensor::eye(1 << size, 1 << size));
}
...
TEST(RMPO, Default) { test_over_integers(2, 5, test_default_mpo<RMPO>); }

TEST(RMPO, LocalMatrix2x2) {
  RTensor x = real(mps::Pauli_x);
  RTensor z = real(mps::Pauli_z);
  RTensor i2 = real(mps::Pauli_id);
  auto mpo = initialize_interactions_mpo<RMPO>({2, 2});
...
TEST(RMPO, LocalMatrix3x2) {
  ...
  RTensor z = real(mps::Pauli_z);
  RTensor i2 = real(mps::Pauli_id);
  auto mpo = initialize_interactions_mpo<RMPO>({2, 2, 2});
  add_local_term(&mpo, z, 0);
  add_local_term(&mpo, x, 1);
...
TEST(CMPO, Default) { test_over_integers(2, 5, test_default_mpo<CMPO>); }

TEST(CMPO, LocalMatrix2x2) {
  CTensor x = mps::Pauli_x;
  CTensor z = mps::Pauli_z;
  CTensor i2 = mps::Pauli_id;
  auto mpo = initialize_interactions_mpo<CMPO>({2, 2});
  add_local_term(&mpo, z, 0);
  add_local_term(&mpo, x, 1);
  EXPECT_CEQ(mpo_to_matrix(mpo), kron2(z, i2) + kron2(i2, x));
}
...
TEST(CMPO, LocalMatrix3x2) {
  ...
  CTensor z = mps::Pauli_z;
  CTensor i2 = mps::Pauli_id;
  auto mpo = initialize_interactions_mpo<CMPO>({2, 2, 2});
  add_local_term(&mpo, z, 0);
  add_local_term(&mpo, x, 1);
  add_local_term(&mpo, x + z, 2);
  EXPECT_CEQ(mpo_to_matrix(mpo), kron2(z, kron2(i2, i2)) +
