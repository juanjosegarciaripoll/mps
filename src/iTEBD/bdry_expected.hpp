...
namespace mps {
  ...
  const Tensor p1(const Tensor &v, const Tensor *A)
  {
    return prop_matrix(v, +1, *A, *A);
  }

  template<class Tensor>
  static Tensor
  left_boundary(const Tensor &A, double tol = 1e-11)
  {
  ...
  template<class Tensor>
  const Tensor p2(const Tensor &v, const Tensor *A, const Tensor *B)
  {
  ...
  template<class Tensor>
  static Tensor
  left_boundary(const Tensor &A, const Tensor &B, double tol = 1e-11)
  {
  ...
  template<class t>
  static inline typename t::elt_t bdry_string_order(const t &Op1, int i, const t &Opmid, const t &Op2, int j, const t &A, const t &lA, const t &B, const t &lB)
  {
     if (i > j)
       return bdry_string_order(Op2, j, Opmid, Op1, i, A, lA, B, lB);
  ...
  template<class t>
  static inline typename t::elt_t bdry_expected(const t &Op1, const t &A, const t &lA)
  {
    t AlA = scale(A, -1, lA);
  ...
  template<class t>
  static inline typename t::elt_t bdry_expected12(const t &Op12, const t &A, const t &lA, const t &B, const t &lB)
  {
    return bdry_expected<t>(Op12, ensure_3_indices<t>(fold(scale(A, -1, lA), -1, B, 0)), lB);
  }

  template<class t>
  static inline typename t::elt_t bdry_expected12(const iTEBD<t> &psi, const t &Op12, int site)
  {
    if (site & 1)
