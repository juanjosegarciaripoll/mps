...
namespace mps {
  ...
  static void
  do_apply_rightwards(MPS &chi, const MPO &mpdo, const MPS &psi,
                      bool truncate)
  {
  ...
  template<class MPS, class MPO>
  static void
  do_apply_leftwards(MPS &chi, const MPO &mpdo, const MPS &psi,
                     bool truncate)
  {
    typedef typename MPS::elt_t Tensor;
    index b = 1, a1, c1, di, dj, c2, a2;
    ...
    for (index ndx = psi.size(); ndx--; ) {
      ...
      C = foldin(reshape(O, c1, dj, di*c2), 2,
                 reshape(C, a1, di*c2, b), 1);
