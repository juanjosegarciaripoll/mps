...
namespace mps {
  ...
  const Indices
  Lattice::states_in_particle_range(int sites, int nmin, int nmax)
  {
    if (sizeof(word) == 4) {
      if (sites >= 32) {
  ...
  static Lattice::word
  find_configuration(Lattice::word w, const Indices &configurations)
  {
  ...
  void
  Lattice::bipartition(int sites_left,
		       Indices *left_states, Indices *right_states,
		       Indices *matrix_indices) const
  {
	...
	"in any bipartition cannot be less than one." << std::endl
