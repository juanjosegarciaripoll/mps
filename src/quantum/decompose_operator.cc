...
static inline void do_decompose_operator(const tensor &U0, tensor *O1,
                                         tensor *O2) {
  /*
  ...
  if (U0.rows() != U0.columns()) {
    std::cerr << "The routine decompose_operator() can only act on square "
                 "matrices and you\n"
              << "have passed a matrix that is " << U0.rows() << " by "
              << U0.columns();
    abort();
  }

  tensor U =
      reshape(permute(reshape(U0, d1, d2, d1, d2), 1, 2), d1 * d1, d2 * d2);
#if 1
