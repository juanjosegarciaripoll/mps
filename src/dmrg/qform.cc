...
namespace mps
{
  ...
  template <class MPO>
  QuadraticForm<MPO>::QuadraticForm(const MPO &mpo, const mps_t &bra, const mps_t &ket, int start) : size_(mpo.size()),
  ...
  QuadraticForm<MPO>::make_matrix_database(const MPO &mpo)
  {
    ...
    output.at(0) = output.at(L) =
        matrix_array_t(1, elt_t::ones(1, 1, 1, 1));
    return output;
  }

  template <class MPO>
  void
  QuadraticForm<MPO>::dump_matrices()
  {
    // We only support open boundary condition problems
